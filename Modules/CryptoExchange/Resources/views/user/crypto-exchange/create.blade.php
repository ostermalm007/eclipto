...
<div class="bg-white remove-p buy-sell-tab pb-40 shadow" id="crypto_exchange_user">
    <div class="navtab-crypto r-pxy-62 pxy-62 pb-0">
        ...
        <p class="mb-0 mobile-font f-14 leading-23 text-gray text-center mt-19 gilroy-medium">{{ __('Exchange crypto from the comfort of your home, quickly, safely with minimal fees. Select the wallet & put the amount you want to exchange.') }}</p>
                        ...
                        <div class="nav-tab-parent d-flex justify-content-center mt-4" styl>
                            <div class="d-flex p-2 border-1p rounded-pill gap-1 bg-white nav-tab-child" id="nav-tab" role="tablist">
                                @if (transactionTypeCheck('crypto_swap'))
                                    <a class="tablink-edit text-gray-100 crypto crypto_swap {{ (isset($exchangeType) && $exchangeType == 'crypto_swap' ? 'tabactive' : '') }}" data-type="crypto_swap" href="#">{{ __('Swap') }}</a>
                                @endif
                                @if (transactionTypeCheck('crypto_buy_sell'))
                                    <a class="tablink-edit text-gray-100 crypto crypto_buy {{ (isset($exchangeType) && $exchangeType == 'crypto_buy' ? 'tabactive' : '') }}" data-type="crypto_buy" href="#">{{ __('Buy') }}</a>
                                    <a class="tablink-edit text-gray-100 crypto crypto_sell {{ (isset($exchangeType) && $exchangeType == 'crypto_sell' ? 'tabactive' : '') }}" data-type="crypto_sell" href="#">{{ __('Sell') }}</a>
                                @endif
                            </div>
                        </div>
                    </nav>
