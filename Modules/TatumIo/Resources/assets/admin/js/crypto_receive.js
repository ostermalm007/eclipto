...
if ($('.content').find('#crypto-receive-create').length) {
    ...
    function checkMerchantAmountValidity(amount, merchantAddress, userAddress, network, priority)
    {
        ...
        if (amount < tatumIoMinLimit.amount[network]) {
            checkMinimumAmount(`${minAmount.replace(':x', tatumIoMinLimit.amount[network] + ' ' + network)}`)
        } else {
        ...
        .done(function(res) {
            ...
            if (res.status == 401) {
                ...
                $('.amount-validation-error').after(`<div class="clearfix"></div>
                    ...
                    <small class="form-text text-muted f-12"><b>*${minWithdrawan.replace(':x', tatumIoMinLimit.amount[network]).replace(':y', network) }</b></small><br/>
                    <small class="form-text text-muted f-12"><b>*${minNetworkFee.replace(':x', tatumIoMinLimit.networkFee[network]).replace(':y', network) }</b></small><br/>
                `);
