...
if ($('.content').find('#crypto-send-create').length) {
    ...
    function checkMerchantAmountValidity(
        ...
        if (amount < tatumIoMinLimit.amount[network]) {
            checkMinimumAmount(
                `${minAmount.replace(
                    ":x",
                    tatumIoMinLimit.amount[network] + " " + network
                )}`
            );
        } else {
    ...
    $(document).on("change", "#user_id", function (e) {
        ...
        $(
            "#merchant-address-div, #merchant-balance-div, #user-address-div, #amount-div, #submit-anchor-div, #priority-div"
        ).remove();
            ...
            .done(function (res) {
                if (res.status == 401) {
                    ...
                    var previousPriority =
                        window.localStorage.getItem("priority");
                    ...
                    $(".amount-validation-error")
                      ...
                      <small class="form-text text-muted f-12"><b>*${minWithdrawan
                          .replace(":x", tatumIoMinLimit.amount[network])
                          .replace(":y", network)}</b></small><br/>
                      <small class="form-text text-muted f-12"><b>*${minNetworkFee
                          .replace(":x", tatumIoMinLimit.networkFee[network])
                          .replace(":y", network)}</b></small><br/>
                  `);
                  ...
                  `);
                    ...
                    $(
                        "#merchant-address, #merchant-balance, #user-address"
                    ).attr("readonly", true);
