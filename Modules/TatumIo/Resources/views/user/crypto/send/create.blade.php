...
<div class="bg-white pxy-62 shadow" id="crypto-send-create">
    ...
    <form method="post" action="{{ route('tatumio.user.crypto_send.confirm') }}" id="crypto-send-form">
        ...
        <div class="mt-20 label-top">
            ...
            <span class="receiver-address-validation-error custom-error"></span>
        </div>
        ...
        <div class="mt-20 label-top">
            ...
            <span class="amount-validation-error custom-error"></span>
        </div>

        <div class="mt-20 label-top">
            <p class="mb-0 text-gray-100 dark-B87 gilroy-regular f-12 mt-2">*{{ __('The amount withdrawn/sent must at least be :x :y.', ['x' => getTatumIoMinLimit('amount', $walletCurrencyCode), 'y' => $walletCurrencyCode]) }}</p>
            <p class="mb-0 text-gray-100 dark-B87 gilroy-regular f-12 mt-2">*{{ __('Please keep at least :x :y for network fees.', ['x' => getTatumIoMinLimit('networkFee', $walletCurrencyCode), 'y' => $walletCurrencyCode]) }}</p>
        </div>
        <!-- End Amount -->
        ...
        <div class="mt-28 param-ref">
            ...
            <div class="avoid-blink">
                <select class="select2 priority" data-minimum-results-for-search="Infinity" id="priority" name="priority">
                    <option {{ old('priority') == 'slow' ? 'selected' : '' }} value="slow">{{ __('Low') }}</option>
                    <option {{ old('priority') == 'medium' ? 'selected' : '' }} value="medium">{{ __('Medium') }}</option>
                    <option {{ old('priority') == 'fast' ? 'selected' : '' }} value="fast">{{ __('High') }}</option>
                </select>
            </div>
        </div>
...
@push('js')
    ...
    <script type="text/javascript">
        ...
        var validateAddressUrl = '{{ route("tatumio.user.crypto_send.validate_address") }}';
        var validateBalanceUrl = '{{ route("tatumio.user.crypto_send.validate_balance") }}';
        var cryptoSendConfirmUrl = '{{ route("tatumio.user.crypto_send.confirm") }}';
