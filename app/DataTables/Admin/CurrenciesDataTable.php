...
class CurrenciesDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($currency) {
                ...
                if ($currency->type == 'crypto_asset') {
                    $edit = (Common::has_permission(auth('admin')->user()->id, 'edit_currency')) ? '<a href="' . url(config('adminPrefix').'/'.strtolower(optional($currency->cryptoAssetSetting)->cryptoProvider->name).'/edit/' . encrypt($currency->code)) . '" class="btn btn-primary rounded-full"><i class="fa fa-edit"></i>Edit</a>' : '';
                    if (optional($currency->cryptoAssetSetting)->cryptoProvider->name == 'BlockIo') {
                        $edit = '';
                    }
                } else {
                    $edit = (Common::has_permission(auth('admin')->user()->id, 'edit_currency')) ? '<a href="' . url(config('adminPrefix').'/settings/edit_currency/' . $currency->id) . '" class="btn btn-primary rounded-full"><i class="fa fa-edit"></i>Edit</a>&nbsp;' : '';
                }

                $delete = (Common::has_permission(auth('admin')->user()->id, 'delete_currency')) && (($currency->type == 'fiat') || ($currency->type == 'crypto'))  ? '<a href="' . url(config('adminPrefix').'/settings/delete_currency/' . $currency->id) . '" class="btn btn-danger rounded-full"><i class="fa fa-trash"></i>Delete</a>&nbsp;' : '';

                // $feesLimit = (Common::has_permission(auth('admin')->user()->id, 'edit_currency') && $currency->status == 'Active') && (($currency->type == 'fiat') || ($currency->type == 'crypto')) ? '<a href="' . url(config('adminPrefix').'/settings/feeslimit/deposit/' . $currency->id) . '" class="btn btn-success rounded-full">

                // <span class="glyphicon glyphicon-view">Fees</i></a>&nbsp;' : '';
                $pm = (Common::has_permission(auth('admin')->user()->id, 'edit_currency')) && $currency->status == 'Active' && (($currency->type == 'fiat') || ($currency->type == 'crypto')) ? '<a href="' . url(config('adminPrefix').'/settings/payment-methods/'.$type.'/' . $currency->id) . '" class="btn btn-info rounded-full">
                    <span class="glyphicon glyphicon-view">Payment-Methods</i></a>&nbsp;' : '';

                return "<div class='flex w-1/2 items-    justify- '>".$edit . $delete . $feesLimit . $pm.'</div>';
            })
    ...
    {
        $query = Currency::with('cryptoAssetSetting:id,currency_id,crypto_provider_id', 'cryptoAssetSetting.cryptoProvider:id,name')->select('id', 'type', 'name', 'code', 'symbol', 'rate', 'logo', 'status', 'default');
