...
class AddressProofController extends Controller
{
    ...
    public function index(AddressProofsDataTable $dataTable)
    {
        ...
        // $rows=DocumentVerification::where(['verification_type' => 'address'])->groupBy('status')->with('user')->paginate(12);
        // return view('admin.verifications.address_proofs.list', compact('rows'));
    ...
    public function addressProofUpdate(Request $request)
    {
        ...
        if ($request->verification_type == 'address') {
            ...
            $status='<span class="badge '.$statuscolor.' ">'.$request->status.'</span>';
