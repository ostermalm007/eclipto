...
class IdentityProofController extends Controller
{
    ...
    public function index(IdentityProofsDataTable $dataTable)
    {
        ...
        // $rows=DocumentVerification::where(['verification_type' => 'identity'])->groupBy('status')->with('user')->paginate(12);

        // return view('admin.verifications.identity_proofs.list', compact('rows'));
    }
    ...
    public function identityProofUpdate(Request $request)
    {
        ...
        if (!empty($documentVerification) && $request->verification_type == 'identity') {
            ...
            $status='<span class="badge '.$statuscolor.' ">'.$data['status'].'</span>';

            return ['status'=>'success','id'=>$request->id,'badge'=>$status];
        }

        return ['status'=>'error','id'=>$request->id,'badge'=>''];
