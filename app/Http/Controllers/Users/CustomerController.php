...
class CustomerController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $totalPendingTransactions=Transaction::where('user_id',Auth::id())->where('status','pending')->count();
        ...
        foreach ($chartLables as $month) {
            ...
            $succeededTransactions = Transaction::where('user_id',Auth::id())->where('status','Success')->whereBetween('created_at', [$startDate, $endDate])->sum('subtotal');
            $chartSucceededTransactions[] = $succeededTransactions;

            $cancledTransactions = Transaction::where('user_id',Auth::id())->where('status','Blocked')->whereBetween('created_at', [$startDate, $endDate])->sum('subtotal');
            $chartCancledTransactions[] = $cancledTransactions;
            
        }

        return view('user.dashboard', compact('data','totalPendingTransactions','chartLables','chartSucceededTransactions','chartCancledTransactions'));
    ...
    public function generatePhoneVerificationCode(Request $request)
    {
        ...
        if ( !checkAppSmsEnvironment() || empty(getSmsConfigDetails()) 
        || getSmsConfigDetails()->status !== 'Active' || empty($request->phone) 
        || empty($request->carrierCode) || empty($request->phone)) {
    ...
    public function updatePersonalAddress(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            $documentVerification->country=$request->country;
            $documentVerification->state=$request->state;
            $documentVerification->city=$request->city;
            $documentVerification->zip_code=$request->zip_code;
            $documentVerification->address_line_1=$request->address_line_1;
            $documentVerification->address_line_2=$request->address_line_2;
            $documentVerification->save();
    ...
    {
        $data['tatum_provider_status'] = CryptoProvider::getStatus('TatumIo');
