          ...
          <div class="col-md-12">
              <div class="box box-default">
                    <div class="d-flex align-items-center justify-content-between p-4">
                      <h3 class="box-title mb-0 f-18">{{ __('Manage Feedbacks') }}</h3>

                      @if(Common::has_permission(auth('admin')->user()->id, 'add_page'))
                        <div><a class="btn btn-theme f-14 float-end" href="{{  url(config('adminPrefix')."/settings/feedback/add") }}">{{ __('Add Feedback') }}</a></div>
                      @endif
                    </div>
                    <hr>
