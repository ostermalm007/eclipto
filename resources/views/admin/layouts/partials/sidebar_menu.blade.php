<style>
    .mx-3.rounded a{border-radius:.375rem;}
</style>
<ul class="sidebar-menu mt-20">
    <li class="treeview mr-3 rounded {{ isset($menu) && $menu == 'dashboard' ? 'active' : '' }}">
        <a href="{{ url(config('adminPrefix').'/home') }}" class="text-dark">
            <i class="fa fa-dashboard text-secondary"></i><span>{{ __('Dashboard') }}</span>
        </a>
    </li>

    <li class="treeview mr-3 rounded {{ isset($menu) && $menu == 'crypto_providers' ? 'active' : '' }}">
        <a href="{{ url(config('adminPrefix').'/crypto-providers/tatumio') }}" class="text-dark">
            <i class="fa fa-paper-plane text-secondary"></i><span>{{ __('Tatum Wallet') }}</span>
        </a>
    </li>

    <li class="treeview mr-3 rounded {{ isset($sub_menu) && $sub_menu == 'transactions' ? 'active' : '' }}">
        <a href="{{ url(config('adminPrefix').'/transactions') }}" class="text-dark">
            <i class="fa fa-exchange text-secondary"></i><span>{{ __('All Transactions') }}</span>
        </a>
    </li>


    <li class="treeview mr-3 rounded {{ isset($sub_menu) && $sub_menu == 'exchange_directions' ? 'active' : '' }}">
        <a href="{{ url(config('adminPrefix').'/exchange/directions') }}" class="text-dark">
            <i class="fa fa-retweet text-secondary"></i><span>{{ __('Exchange Directions') }}</span>
        </a>
    </li>
    <li class="treeview mr-3 rounded {{ isset($menu) && $menu == 'currency' ? 'active' : '' }}">
        <a href="{{ url(config('adminPrefix').'/settings/currency') }}" class="text-dark">
            <i class="fa fa-usd text-secondary"></i><span>{{ __('Currencies') }}</span>
        </a>
    </li>
    {{-- <li class="treeview mr-3 rounded {{ isset($menu) && $menu == 'crypto-providers/tatumio' ? 'active' : '' }}">
        <a href="{{ url(config('adminPrefix').'/crypto-providers/tatumio') }}" class="text-dark">
            <i class="fa fa-usd text-secondary"></i><span>{{ __('Tatum Assets') }}</span>
        </a>
    </li> --}}
    <li class="treeview mr-3 rounded {{ isset($menu) && $menu == 'blogs' ? 'active' : '' }}">
        <a href="{{ url(config('adminPrefix').'/blogs') }}" class="text-dark">
            <i class="fa fa-file-text text-secondary"></i><span>{{ __('Blogs') }}</span>
        </a>
    </li>
    <li class="treeview mr-3 rounded {{ isset($menu) && $menu == 'pages' ? 'active' : '' }}">
        <a href="{{ url(config('adminPrefix').'/settings/pages') }}" class="text-dark">
            <i class="fa fa-file text-secondary"></i><span>{{ __('Pages') }}</span>
        </a>
    </li>
    <li class="treeview mr-3 rounded {{ isset($menu) && $menu == 'feedbacks' ? 'active' : '' }}">
        <a href="{{ url(config('adminPrefix').'/settings/feedbacks') }}" class="text-dark">
            <i class="fa fa-user text-secondary"></i><span>{{ __('Feedbacks') }}</span>
        </a>
    </li>
    ...
    @if(Common::has_permission(auth()->guard('admin')->id(), 'view_identity_verfication') || Common::has_permission(auth()->guard('admin')->id(), 'view_address_verfication'))
        <li class="treeview mr-3 rounded {{ (isset($menu) && $menu == 'proofs') ? 'menu-open active' : '' }}">
            <a href="#" class="text-dark">
                <i class="fa fa-check-square-o text-secondary"></i><span>{{ __('Verifications') }}</span>
                <span class="pull-right-container text-secondary"><i class="fa fa-angle-left pull-right"></i></span>
            </a>
            <ul class="treeview-menu">
                @if(Common::has_permission(auth()->guard('admin')->id(), 'view_identity_verfication'))
                    <li class="child {{ isset($sub_menu) && $sub_menu == 'identity-proofs' ? 'active' : '' }}">
                        <a href="{{ url(config('adminPrefix').'/identity-proofs') }}" class="text-dark">
                            <i class="fa fa-user-circle-o text-secondary"></i><span>{{ __('Identity Verification') }}</span>
                        </a>
                    </li>
                @endif
                ...
                @if(Common::has_permission(auth()->guard('admin')->id(), 'view_address_verfication'))
                    <li class="child {{ isset($sub_menu) && $sub_menu == 'address-proofs' ? 'active' : '' }}">
                        <a href="{{ url(config('adminPrefix').'/address-proofs') }}" class="text-dark">
                            <i class="fa fa-address-book text-secondary"></i><span>{{ __('Address Verification') }}</span>
                        </a>
                    </li>
                @endif
            </ul>
    ...
    <li class="treeview mr-3 rounded {{ isset($sub_menu) && $sub_menu == 'users_list' ? 'active' : '' }}">
        <a href="{{ url(config('adminPrefix').'/users') }}" class="text-dark">
            <i class="fa fa-users text-secondary"></i><span>{{ __('Users') }}</span>
        </a>
    </li>

    {{-- <!--users-->
    @if(Common::has_permission(auth()->guard('admin')->id(), 'view_user') || Common::has_permission(auth()->guard('admin')->id(), 'view_admins'))
        <li class="treeview mr-3 rounded {{ (isset($menu) && $menu == 'users') ? 'menu-open active' : '' }}">
            <a href="#">
    ...
    @if(Common::has_permission(auth()->guard('admin')->id(), 'view_deposit') || Common::has_permission(auth()->guard('admin')->id(), 'view_withdrawal') || Common::has_permission(auth()->guard('admin')->id(), 'view_transfer') || Common::has_permission(auth()->guard('admin')->id(), 'view_exchange') || Common::has_permission(auth()->guard('admin')->id(), 'view_request_payment') || Common::has_permission(auth()->guard('admin')->id(), 'view_transaction'))
        <li class="treeview mr-3 rounded {{ (isset($menu) && $menu == 'transaction') ? 'menu-open active' : '' }}">
            <a href="#">
            ...
            <ul class="treeview-menu">
                ...
                @endif
                @if (Common::has_permission(auth()->guard('admin')->id(), 'view_crypto_transactions'))
    ...
    <li class="child {{ isset($menu) && $menu == 'crypto_providers' ? 'active' : '' }}">
        <a href="{{ route('admin.crypto_providers.list', 'TatumIo') }}">
    ...
    <!-- settings -->
    <li class="treeview mr-3 rounded {{ isset($menu) && $menu == 'settings' ? 'active' : '' }}">
    ...
    <!-- Cache Clear -->
    @if(Common::has_permission(auth()->guard('admin')->id(), 'view_cache_clear'))
    <li>
        <a href="{{ route('clear.cache') }}"><i class="fa fa-trash"></i><span>{{ __('Cache Clear') }}</span></a>
    </li>
    @endif --}}
</ul>