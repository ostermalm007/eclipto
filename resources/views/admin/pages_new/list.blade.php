...
@section('buttons')
  @if(Common::has_permission(auth('admin')->user()->id, 'add_page'))
    <a class="btn btn-primary rounded-full float-right" href="{{  url(config('adminPrefix')."/settings/page/add") }}">{{ __('Add Page') }}</a>
  @endif
@endsection
            ...
            <tbody>
              @forelse($rows as $row)
                <tr>
                  ...
                  <td>
                    <a target="_blank" class="text-primary hover:underline" href="{{ url('/'.$row['url']) }}">
                      {{ $row['url'] }}
                    </a>
                  </td>
                  <td>{{ json_encode($row['position']) }}</td>
                  <td>
                    <span class="badge bg-{{ $row['status']=='active'?'success':'danger' }}">{{ $row['status'] }}</span>
                  </td>
                  <td>
                    <a href="{{ url(config('adminPrefix') . '/settings/page/edit/' . $row['id']) }}" class="btn btn-xs btn-primary">
                      <i class="fa fa-edit"></i> Edit
                    </a>
                    <a href="{{ url(config('adminPrefix') . '/settings/page/delete/' . $row['id']) }}" class="btn btn-xs btn-danger delete-warning">
