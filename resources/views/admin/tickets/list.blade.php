...
@section('page_content')
    <div class="box">
        <div class="box-body pb-20">
            <form class="form-horizontal" action="{{ url(config('adminPrefix').'/tickets/list') }}" method="GET">
    ...
    <div class="box box-default">
        <div class="box-body">
            <div class="d-flex justify-content-between">
                <div>
                    <div class="top-bar-title padding-bottom pull-left">{{ __('Tickets') }}</div>
                </div>
                <div>
                    @if(Common::has_permission(auth('admin')->user()->id, 'add_ticket'))
                        <a href="{{ url(config('adminPrefix').'/tickets/add') }}" class="btn btn-theme f-14"><span class="fa fa-plus f-14"> &nbsp;</span>{{ __('Add Ticket') }}</a>
                    @endif
                </div>
            </div>
        </div>
                ...
                <div class="col-md-2">
                    <div class="panel panel-primary">
                        <div class="panel-body text-center">
                            ...
                            <span class="f-14" style="font-weight: bold; color: #{{ $color }} ;">{{ $data->name }}</span>
                        </div>
                     </div>
                </div>
            @endforeach
