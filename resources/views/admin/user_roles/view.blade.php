    ...
    <div class="col-md-9">
        <div class="box box_info">
              <div class="d-flex align-items-center justify-content-between p-4">
                <h3 class="box-title mb-0 f-18">{{ __('Manage User Group') }}</h3>
                @if(Common::has_permission(auth('admin')->user()->id, 'add_group'))
                  <div><a class="btn btn-theme f-14 float-end" href="{{ url(config('adminPrefix').'/settings/add_user_role') }}">{{ __('Add Group') }}</a></div>
                @endif
              </div>
              <hr>
