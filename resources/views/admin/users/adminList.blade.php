    ...
    <div class="box box-default">
        <div class="box-body">
            <div class="d-flex justify-content-between">
                <div>
                    <div class="top-bar-title padding-bottom pull-left">{{ __('Admins') }}</div>
                </div>

                <div>
                    @if(Common::has_permission(auth('admin')->user()->id, 'add_admin'))
                        <a href="{{url(config('adminPrefix').'/admin-user/create')}}" class="btn btn-theme f-14"><span class="fa fa-plus"> &nbsp;</span>{{ __('Add Admin') }}</a>
                    @endif
                </div>
            </div>
        </div>
