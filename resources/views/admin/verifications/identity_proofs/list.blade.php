...
@section('page_content')
    <div class="box">
        <div class="box-body pb-20">
            <form class="form-horizontal" action="{{ url(config('adminPrefix').'/identity-proofs') }}" method="GET">
    ...
    <div class="row">
        <div class="col-md-8">
            <p class="panel-title text-bold ml-5 mb-0 f-14">{{ __('All Identity Verifications') }}</p>
        </div>
        <div class="col-md-4">
...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
        $('#csv').on('click', function(event)
        {
          ...
          window.location = SITE_URL+"/"+ADMIN_PREFIX+"/identity-proofs/csv?startfrom="+startfrom
          +"&endto="+endto
          +"&status="+status
        });
    });
    ...
    $(document).ready(function()
    {
        $('#pdf').on('click', function(event)
        {
          ...
          window.location = SITE_URL+"/"+ADMIN_PREFIX+"/identity-proofs/pdf?startfrom="+startfrom
          +"&endto="+endto
          +"&status="+status
        });
    });
