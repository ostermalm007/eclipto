...
@section('page_content')
    <div class="box">
        <div class="box-body pb-20">
            <form class="form-horizontal" action="{{ url(config('adminPrefix').'/withdrawals') }}" method="GET">
    ...
    <div class="row">
        <div class="col-md-8">
            <p class="panel-title text-bold ml-5 mb-0 f-14">{{ __('All Payouts') }}</p>
        </div>
        <div class="col-md-4">
...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
        ...
        $('#user_input').autocomplete(
        {
            source:function(req,res)
            {
                ...
                {
                    $.ajax({
                        url:'{{url(config('adminPrefix').'/withdrawals/user_search')}}',
                        dataType:'json',
                        type:'get',
                        data:{
                            search:req.term
    ...
    $(document).ready(function()
    {
        $('#csv').on('click', function(event)
        {
          ...
          window.location = SITE_URL+"/"+ADMIN_PREFIX+"/withdrawals/csv?startfrom="+startfrom
          +"&endto="+endto
          +"&status="+status
          +"&currency="+currency
    ...
    $(document).ready(function()
    {
        $('#pdf').on('click', function(event)
        {
          ...
          window.location = SITE_URL+"/"+ADMIN_PREFIX+"/withdrawals/pdf?startfrom="+startfrom
          +"&endto="+endto
          +"&status="+status
          +"&currency="+currency
          +"&payment_methods="+payment_methods
