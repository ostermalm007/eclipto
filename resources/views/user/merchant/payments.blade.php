...
<div class="text-center">
    <p class="mb-0 gilroy-Semibold f-26 text-dark theme-tran r-f-20">{{ __('Payments') }}</p>
...
<div class="mt-22 mt-sm-4" id="merchantPayment">
    <div class="d-flex justify-content-between align-items-center r-pb-8 pb-10">
        <p class="mb-0 text-gray-100 f-16 r-f-12 gilroy-medium dark-CDO">{{ __('All lists of merchant payment') }}</p>
        ...
        <div class="filter-panel">
            <div class="d-flex flex-wrap justify-content-between pb-26">
                <div class="d-flex flex-wrap align-items-center pb-2 pb-xl-0">
                    ...
                    <div class="me-2">
                        <div class="param-ref filter-ref w-135">
                            ...
                            data-minimum-results-for-search="Infinity" id="currency" name="currency">
                                <option value="all" {{ $currency == 'all' ? 'selected' : '' }}>{{ __('All Currency')  }}</option>
                                @foreach ($currencies as $item)
                                    <option value="{{ $item->currency_id }}" {{ $currency == $item->currency_id ? 'selected' : '' }}> {{ $item->currency?->code }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="me-2">
                        <div class="param-ref filter-ref w-135">
                            <select class="select2 f-13"
                            data-minimum-results-for-search="Infinity" id="paymentMethod" name="paymentMethod">
                                <option value="all" {{ $paymentMethod == 'all' ? 'selected' : '' }}>{{ __('All Method')  }}</option>
                                @foreach ($paymentMethods as $item)
                                    <option value="{{ $item->payment_method_id }}" {{ $paymentMethod == $item->payment_method_id ? 'selected' : '' }}> {{ $item->payment_method?->name == 'Mts' ? settings('name'): $item->payment_method?->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="param-ref filter-ref w-135">
                            ...
                            data-minimum-results-for-search="Infinity" id="status" name="status">
                                ...
                                @foreach ($statuses as $item)
                                    <option value="{{ $item->status }}" {{ $status == $item->status ? 'selected' : '' }}> {{ $item->status == 'Refund' ? __('Refunded') : $item->status }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="me-2">
                        <div class="param-ref filter-ref w-135">
                            <select class="select2 f-13"
                            data-minimum-results-for-search="Infinity" id="merchant" name="merchant">
                                <option value="all" {{ $merchant == 'all' ? 'selected' : '' }}>{{ __('All Merchant')  }}</option>
                                @foreach ($merchants as $item)
                                    <option value="{{ $item->id }}" {{ $merchant == $item->id ? 'selected' : '' }}> {{ $item->business_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="d-flex align-items-center pb-2 pb-xl-0">
                    <a href="{{ route('user.merchants.payments') }}" class="reset-btn text-gray-100 f-14 gilroy-medium leading-17 tran-title">{{ __('Reset') }}</a>
...
<div class="row mt-48 mb-0">
    <div class="col-md-12">
        @if($merchantPayments->count() > 0)
            <div class="table-responsive table-scrolbar thin-scrollbar">
                <table class="merchant-payments table-curved  table recent_activity table-bordered">
                    <thead>
                        <tr class="payment-parent-section-title component-table-one">
                            ...
                            <th class="p-0 pb-6">
                                <p class="mb-0 f-14 leading-17 gilroy-regular text-gray-100">{{ __('Merchant') }}</p>
                            </th>
                            <th class="p-0 pb-6">
                                <p class="mb-0 f-14 leading-17 gilroy-regular text-gray-100">{{ __('Payer') }}</p>
                            </th>
                            <th class="p-0 pb-6">
                                <p class="mb-0 f-14 leading-17 gilroy-regular text-gray-100">{{ __('Method') }}</p>
                            </th>
                            <th class="p-0 pb-6">
                                <p class="mb-0 f-14 leading-17 gilroy-regular text-gray-100">{{ __('Order Number') }}</p>
                            </th>
                            <th class="p-0 pb-6">
                                <p class="mb-0 f-14 leading-17 gilroy-regular text-gray-100">{{ __('Amount') }}</p>
                            </th>
                            <th class="p-0 pb-6">
                                <p class="mb-0 f-14 leading-17 gilroy-regular text-gray-100">{{ __('Fee') }}</p>
                            </th>
                            <th class="p-0 pb-6">
                                <p class="mb-0 f-14 leading-17 gilroy-regular text-gray-100">{{ __('Total') }}</p>
                            </th>
                            <th class="p-0 pb-6">
                                <p class="mb-0 f-14 leading-17 gilroy-regular text-gray-100">{{ __('Currency') }}</p>
                            </th>
                            <th class="p-0 pb-6">
                    ...
                    <tbody>
                        @foreach($merchantPayments as $result)
                            <tr class="bg-white">
                                <td>
                                    <p class="mb-0 f-14 leading-17 gilroy-medium text-dark">{{ dateFormat($result->created_at) }}</p>
                                </td>
                                <td>
                                    <p class="mb-0 f-14 leading-17 gilroy-medium text-dark">{{ $result->merchant?->business_name }}</p>
                                </td>
                                <td>
                                    <p class="mb-0 f-14 leading-17 gilroy-medium text-dark">{{ getColumnValue($result->user) }}</p>
                                </td>
                                <td>
                                    <p class="mb-0 f-14 leading-17 gilroy-medium text-dark">{{ ($result->payment_method?->name == "Mts") ? settings('name') : $result->payment_method?->name }}</p>
                                </td>
                                <td>
                                    <p class="mb-0 f-14 leading-17 gilroy-medium text-dark">{{ !empty($result->order_no) ? $result->order_no : "-" }}</p>
                                </td>
                                <td>
                                    <p class="mb-0 f-14 leading-17 gilroy-medium text-dark">{{ formatNumber($result->amount)}}</p>
                                </td>
                                <td>
                                    <p class="mb-0 f-14 leading-17 gilroy-medium text-dark">{{ (($result->charge_percentage + $result->charge_fixed) == 0 || $result->fee_bearer == 'User') ? '-' : formatNumber($result->charge_percentage + $result->charge_fixed) }}</p>
                                </td>
                                <td>
                                    <p class="mb-0 f-14 leading-17 gilroy-medium text-dark">{{ $result->fee_bearer == 'User' ? formatNumber($result->amount) :formatNumber($result->total) }}</p>
                                </td>
                                <td>
                                    <p class="mb-0 f-14 leading-17 gilroy-medium text-dark">{{ $result->currency?->code}}</p>
                                </td>
                                <td>
                                    <p class="mb-0 f-14 leading-17 gilroy-medium {{ getColor($result->status) }}">{{ $result->status }}</p>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
<div class="p-0">
    {{ $merchantPayments->appends(['from' => request()->from, 'to' => request()->to, 'status' => request()->status, 'currency' => request()->currency, 'paymentMethod' => request()->paymentMethod, 'orderNumber' => request()->orderNumber, 'merchant' => request()->merchant])->links('vendor.pagination.bootstrap-5') }}
</div>
@endsection
