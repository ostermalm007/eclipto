            ...
            <nav>
                <div class="nav-tab-parent d-flex justify-content-center mt-4">
                    <div class="d-flex p-2 border-1p rounded-pill gap-1 bg-white nav-tab-child">
                        ...
                        <a href="{{ route('user.setting.address_verify') }}" class="tablink-edit text-gray-100 @if(empty(App\Models\DocumentVerification::where(['user_id' => $user->id, 'verification_type' => 'address', 'status' => 'approved'])->first())) verification-alert-active @endif">{{ __('Address Verfication') }}</a>
