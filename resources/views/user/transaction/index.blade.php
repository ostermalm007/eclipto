...
<div class="text-center">
    <p class="mb-0 gilroy-Semibold f-26 text-dark theme-tran r-f-20 text-uppercase">{{ __('Transactions') }}</p>
    <p class="mb-0 gilroy-medium text-gray-100 f-16 r-f-12 l-sp64  mt-2 tran-title">{{ __('History of transactions in your account') }}</p>
</div>
@if($transactions->count() > 0)
    <div class="mt-22 mt-sm-4">
        <div class="d-flex justify-content-between align-items-center r-pb-8 pb-10">
            <p class="mb-0 text-gray-100 f-16 r-f-12 gilroy-medium dark-CDO">{{ __('All Transactions') }}</p>
            ...
            <div class="filter-panel">
                <div class="d-flex flex-wrap justify-content-between pb-26">
                            ...
                            <div class="param-ref filter-ref w-135 h-45">
                                <select class="select2 f-13" data-minimum-results-for-search="Infinity" id="type" name="type">
                                    ...
                                    @foreach ($transactionTypes as $transactionTypeKey => $transactionTypeName)
                                        <option value="{{ $transactionTypeKey }}" {{ $type == $transactionTypeKey ? 'selected' : '' }}>{{ strpos($transactionTypeName, '_') !== false ? str_replace('_', ' ', $transactionTypeName) : $transactionTypeName }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="param-ref filter-ref w-135 h-45">
                                <select class="select2 f-13" data-minimum-results-for-search="Infinity" id="status" name="status">
                                    ...
                                    @foreach ($statuses as $value)
                                        <option value="{{ $value->status }}" {{ $status == $value->status ? 'selected' : '' }}>{{ getStatus($value->status) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="param-ref filter-ref w-135 h-45">
                                <select class="select2 f-13" data-minimum-results-for-search="Infinity" id="wallet" name="wallet">
                                    ...
                                    @foreach($wallets as $value)
                                        <option value="{{ $value->currency->id }}" {{ $value->currency_id == $wallet ? 'selected' : '' }}>{{ $value->currency?->code }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="d-flex align-items-center p-2">
                        <a href="{{ route('user.transactions.index') }}" class="reset-btn text-gray-100 f-14 gilroy-medium leading-17 tran-title">{{ __('Reset') }}</a>
...
@push('js')
    ...
    <script src="{{ asset('public/user/customs/js/transaction.min.js') }}"></script>
@endpush